...
def desugar_file(file_to_desugar: Path,
    ...
    if config_prefix != None:
        cmd = ['/usr/bin/time', '-v', 'timeout -k 10 10m', 'java', '-Xmx4g', 'superc.SugarC', '-showActions', '-useBDD', '-restrictConfigToPrefix', config_prefix, *commandline_args, *included_files, *included_directories,file_to_desugar]
    elif whitelist != None:
        cmd = ['/usr/bin/time', '-v', 'timeout -k 10 10m', 'java', '-Xmx4g', 'superc.SugarC', '-showActions', '-useBDD', '-restrictConfigToWhitelist', whitelist, *commandline_args, *included_files, *included_directories,file_to_desugar]
    else:
        cmd = ['/usr/bin/time', '-v', 'timeout -k 10 10m', 'java', '-Xmx4g', 'superc.SugarC', '-showActions', '-useBDD', *commandline_args, *included_files, *included_directories,file_to_desugar]
...
def process_alarms(alarms: Iterable[Alarm], desugared_file: Path) -> Iterable[Alarm]:
    ...
    for w in alarms:
        ...
        for a in w.static_condition_results:
            if (a['var'] == ''
                    or not a['var'] in condition_mapping.replacers.keys()
                    or condition_mapping.replacers[a['var']] == ''):
